                                         ...
                                         double& height)
{
    ...
    {
        my_height += (x - nearPoints.at(i)).cross(nearNormals.at(i));
    }
    height = my_height;
    //end of TODO
...
Eigen::Vector2d IMLSICPMatcher::ComputeNormal(std::vector<Eigen::Vector2d> &nearPoints)
{
    ...
    for(std::vector<Eigen::Vector2d>::iterator i = nearPoints.begin();i != nearPoints.end();i++){
        sum += *i;
    }
    // sum = sum / (nearPoints.size());

    // Eigen::Matrix2d m_sum(0,0,0,0);
    // for(std::vector<Eigen::Vector2d>::iterator i = nearPoints.begin();i != nearPoints.end();i++){
    //     m_sum += ((*i - sum).transpose()) * (*i - sum) / (nearPoints.size());
    // }
    // Eigen::EigenSolver<Eigen::Matrix2d> eigen_solver(m_sum);

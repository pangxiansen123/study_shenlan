...
{
    // 把当前帧的数据赋值给m_sourcePointCloud
    m_sourcePointCloud = _source_pcloud;

    //去除非法数据，还是在m_sourcePointCloud里面
                                         ...
                                         double &height)
{
    ...
    // target_point = nearPoints.front();
    // height = nearNormals.front().cross(target_point - x);
 ...
 * @param out_cloud         当前帧的匹配点云 就是当前点求出来的匹配点云
 * @param out_normal        匹配点云对应的法向量． 就是当前点最近点的法向量
 */
void IMLSICPMatcher::projSourcePtToSurface(
    std::vector<Eigen::Vector2d> &in_cloud, //对当前帧的点云进行了转换得到的点云
    std::vector<Eigen::Vector2d> &out_cloud, //就是当前点求出来的匹配点云
    std::vector<Eigen::Vector2d> &out_normal) //就是当前点最近点的法向量
{
    out_cloud.clear();
    out_normal.clear();
...
Eigen::Vector2d IMLSICPMatcher::ComputeNormal(std::vector<Eigen::Vector2d> &nearPoints)
{
    ...
    cova.setZero();
    for(int i = 0; i < ptCnt; i++){
        Eigen::Vector2d pt = nearPoints[i] - sum;
        cova(0, 0) += pt(0) * pt(0);
        cova(1, 1) += pt(1) * pt(1);
        cova(0, 1) += pt(0) * pt(1);
        cova(1, 0) += pt(1) * pt(0);
    ...
    cova(0, 1) /= ptCnt;
    cova(1, 0) /= ptCnt;
    // Eigen::EigenSolver<Eigen::Matrix2d> eigen_solver(cova);
    // Eigen::Matrix2Xcd evecs = eigen_solver.eigenvectors();
    // Eigen::Matrix2Xcd evals = eigen_solver.eigenvalues();
    // Eigen::Matrix2Xcd evalsReal;
    // evalsReal = evals.real();
    // Eigen::Matrix2d::Index evalsMin;
    // evalsReal.rowwise().sum().minCoeff(&evalsMin);
    // normal << evecs.real()(0,evalsMin),evecs.real()(0,evalsMin);
    Eigen::SelfAdjointEigenSolver<Eigen::Matrix2d> eig(cova);
                           ...
                           Eigen::Matrix3d &covariance)
{
    ...
    if (m_isGetNormals == false)
    {
        ...
        for (int i = 0; i < m_targetPointCloud.size(); i++)
        {
            ...
            // 本帧的所有点云的法向量存储在m_targetPtCloudNormals
    ...
    for (int i = 0; i < m_Iterations; i++)
    {
        ...
        projSourcePtToSurface(in_cloud,  //对当前帧的点云进行了转换得到的点云
                              ref_cloud, //当前帧的匹配点云 就是当前点求出来的匹配点云
                              ref_normal); //匹配点云对应的法向量． 就是当前点最近点的法向量
